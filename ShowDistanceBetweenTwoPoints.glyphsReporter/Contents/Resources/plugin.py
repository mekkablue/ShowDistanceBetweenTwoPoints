...
from Cocoa import NSBezierPath, NSColor, NSString, NSRect, NSPoint, NSSize, NSNotFound
...
class ShowDistanceBetweenTwoPoints(ReporterPlugin):
	...
	def foreground(self, layer):
		...
		if currentSelection and len(currentSelection) == 2:
			...
			if isinstance(node1, GSNode) and isinstance(node2, GSNode):
				# set drawing color:
				...
				if Glyphs.buildNumber == 965:
					...
					distanceNSString.drawBadgeAtPoint_size_color_backgroundColor_alignment_visibleInRect_(middle, scaledSize, fontColor, drawingColor, 3, NSRect((NSNotFound, 0), (0, 0)))
				else:
